...
class PacketManager
{
	...
	{		
		...
		_onRecv.Add((ushort)MsgId.CStageClear, MakePacket<C_StageClear>);
		_handler.Add((ushort)MsgId.CStageClear, PacketHandler.C_StageClearHandler);		
		...
		_onRecv.Add((ushort)MsgId.CRaidMatch, MakePacket<C_RaidMatch>);
		_handler.Add((ushort)MsgId.CRaidMatch, PacketHandler.C_RaidMatchHandler);		
		_onRecv.Add((ushort)MsgId.CQuestChange, MakePacket<C_QuestChange>);
		_handler.Add((ushort)MsgId.CQuestChange, PacketHandler.C_QuestChangeHandler);		
		_onRecv.Add((ushort)MsgId.CGetExp, MakePacket<C_GetExp>);
		_handler.Add((ushort)MsgId.CGetExp, PacketHandler.C_GetExpHandler);		
		_onRecv.Add((ushort)MsgId.CBossStatChange, MakePacket<C_BossStatChange>);
		_handler.Add((ushort)MsgId.CBossStatChange, PacketHandler.C_BossStatChangeHandler);
	}
